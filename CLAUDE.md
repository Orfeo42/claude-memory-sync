...
- **`cmd/server`** — hub API on the always-on home box. `internal/api` (huma v2 on net/http, bearer-token auth middleware) over `internal/store`: a git-backed filesystem store (`gitStore`) that shells out to `git` (`store/git.go`), one commit per write/delete, branch `storage`. Layout under the storage root: `canonical/` (merged truth) and `clients/<client-id>/` (per-machine snapshots). Client writes/deletes **dual-write to `canonical/`** in the same commit (last-writer-wins per file) — that is how knowledge flows between machines; agents down-sync from `canonical/` only.
...
- **`build/synthesizer/`** — third service (shell + Claude Code CLI, no Go): daily condense/dedup pass over `canonical/projects/*/memory/` on the shared storage volume. Subscription auth via `CLAUDE_CODE_OAUTH_TOKEN` (`claude setup-token`); refuses to start with `ANTHROPIC_API_KEY` set. One git commit per project change (post-hoc audit).
