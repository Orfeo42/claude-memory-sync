...
./test/e2e.sh                       # end-to-end test — requires Docker + Go, builds server/agent images + memoryctl
```
...
Hub-and-spoke sync of Claude Code memory files (`~/.claude` content) across machines, **git-native since v2** (design record: `.docs/08-git-native-sync.md`). This repo is **code only** — memory content never lives here. Three Go binaries + a Windmill-hosted synthesis pipeline, all containerized (`build/*.Dockerfile`, `deploy/server/compose.yaml`):
...
- **`cmd/memoryctl`** — CLI with all synthesis-pipeline mechanics, baked into the Windmill worker image (`build/windmill-worker.Dockerfile`): `clients-scan`, `config-merge`, `ops-apply` (parses the `===FILE:/===DELETE:/===PROPOSAL:/NOTHING` op contract from stdin; guardrails via `internal/guardrails`; proposal targets whitelisted to `global/**` and `projects/<key>/memory/*.md`), `proposal-apply`, `commit`/`push` (flock on `/data/state/canonical.lock`), `markers-advance`, `hotcache-write`, `context`, `corpus list|read|search|digest`.
- **Windmill pipeline** (`deploy/windmill/`) — replaces the retired shell synthesizer services. Three flows (`flows/*.flow.yaml`, deployed to the Windmill instance on the server box): **intake** (45m: per-client diff since marker ref `refs/intake/last-processed`, mechanical last-writer-wins config merge, LLM novelty judgment per project), **daily-synth** (03:00: per-project condense/dedup, cross-project distillation with human-gated proposals, hot-cache digest), **taskmine** (weekly: mines the corpus for proposals classified as `skill|rule|code-change|bug|one-time-task`; skills/rules target `global/**`, the rest become per-project memory entries). Proposals stage under `/data/state/proposals/<root-flow-job-id>/` and suspend the run for approval. Prompt text and parameters live in Windmill variables (`f/memory/*`, defaults in `prompts/*.md`) bound via static `$var:` input transforms. All LLM calls are `claude -p` with subscription auth (`CLAUDE_CODE_OAUTH_TOKEN`); every step refuses to run with `ANTHROPIC_API_KEY` set. One git commit per logical change (post-hoc audit). `deploy/windmill/app/` is a full-code React approval-inbox app (per-item approve/reject, per-proposal LLM chat with revision apply); its own lint toolchain lives in that directory (`npm run lint`, `tsc --noEmit`, backend runnables gated by `mypy --strict` + ruff). Setup runbook: `deploy/windmill/setup.md`.
...
- Secret patterns have a single source: Go (`internal/githook/secrets.go`, reused by `internal/guardrails`) — the old shell duplicate died with `build/synthesizer/`.
